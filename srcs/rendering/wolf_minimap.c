...
/*   Updated: 2019/04/09 20:22:06 by tmaluh           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
								...
								fpoint pos, int32_t color)
{
	...
	exep = (point){size * 2 + (size * (pos.y + 1)),
			size * 2 + (size * (pos.x + 1))};
	while (exep.y > ++(p.y) && ((p.x = xstart) ? p.x : !p.x))
		while (exep.x > p.x)
...
{
	while (true == wolf_is_tile(env->map, p)
...
void		wolf_rendering_minimap(t_env *env)
{
	...
	while (env->map->size.y > ++(p.y)&& (p.x = -1))
		while (env->map->size.x > ++(p.x))
			if (false == wolf_is_tile(env->map, (fpoint){p.y, p.x}))
